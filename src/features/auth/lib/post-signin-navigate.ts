...
// import { isLpRole } from '@/lib/role-capabilities'; // TODO(kaxp): Commenting out the flows for now
...
export function nextRouteForUser(_me: Pick<AuthMeResponse, 'role' | 'profile_complete'>): string {
  // TODO(kaxp): Commenting out the flows for now
  // if (!_me.profile_complete) return '/onboarding/profile';
  return '/dashboard';
}
